...
function Reviews() {
  ...
  // Function to navigate back to the previous review
  const backBtnClick = () => {
    setReview(review <= 0 ? reviewsLength : review - 1);
    handleReviewsUpdation();
  };

  // Function to navigate to the newer review
  const frontBtnClick = () => {
    setReview(review >= reviewsLength ? 0 : review + 1);
    handleReviewsUpdation();
  };

  // Function to update the current review
  ...
  // Call the function to update the reviews
  handleReviewsUpdation();
